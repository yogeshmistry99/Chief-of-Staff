      ...
      <CollapsibleSection title="How to Use Life OS" subtitle="The rhythm — glance once, internalise">
        <div className="bg-white border border-[#CAC4D0] rounded-2xl px-4 py-3 mt-2 space-y-3">
          {[
            ...
            { cadence: 'When building the app', action: 'Strategic direction here in Claude.ai (CoS). Implementation prompts pasted into Claude Code. Never mix the two in one session. Keep Claude Code sessions short and focused. After each Claude Code session: paste the summary back to the CoS so the knowledge base stays current. Claude Code updates STATUS.md automatically — if it doesn’t, prompt it before closing the session.' },
            { cadence: 'Where to talk', action: 'CoS (home screen) — cross-bucket, strategic, daily priorities, task triage. Head chats (inside each bucket) — specialist questions scoped to that bucket only. Discussions (inside each bucket) — work one problem, reach a decision, close it. Tasks get created as outcomes.' },
            { cadence: 'If the ranking is wrong', action: 'Tap the task in the computed preview to see its scores. Correct whichever dimension is off — every correction is permanent. If you just need something at the top right now regardless of scores, pin it.' },
          ].map(({ cadence, action }) => (
